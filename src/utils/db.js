...
export const insertPriceItem = (token, round, price, timestamp) => {
  db.serialize(() => {
    db.run(
      `INSERT INTO prices(token, round, value, timestamp)
        VALUES('${token}', '${round}', '${price}', '${timestamp}')`,
      (ret, err) => {
        if (err) console.error(err.message)
      }
    )
...
export const insertFeeItem = (token, usdAmount, tokenAmount) => {
  ...
  db.serialize(() => {
    db.run(
      `INSERT INTO fees(token, usd_amount, token_amount, timestamp)
        VALUES('${token}', '${usdAmount}', '${tokenAmount}', '${timestamp}')`,
      (ret, err) => {
        if (err) console.error(err.message)
      }
    )
...
export const insertVolumeItem = (contract, event, account, token, usdAmount, tokenAmount, isLong, key) => {
  ...
  db.serialize(() => {
    db.run(
      `INSERT INTO volumes(contract, event, key, account, token, usd_amount, token_amount, is_long, timestamp)
        VALUES('${contract}', '${event}', '${key}', '${account}', '${token}', '${usdAmount}', '${tokenAmount}', '${isLong}', '${timestamp}')`,
      (ret, err) => {
        if (err) console.error(err.message)
      }
    )
...
export const getTotalLongPosition = async () => {
  const query = `SELECT *, (increase_vol - decrease_vol) AS long_vol FROM 
            (SELECT SUM(token_amount/1e18)*1e18 AS increase_vol FROM volumes 
                WHERE is_long = 1 AND closed = 0 AND event = 'IncreasePosition')
            LEFT OUTER JOIN 
            (SELECT SUM(token_amount/1e18)*1e18 AS decrease_vol FROM volumes 
